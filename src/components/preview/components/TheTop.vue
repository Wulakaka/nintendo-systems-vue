...
function update(a) {
  ...
  let y = view.value.offsetTop,
    height = view.value.offsetHeight,
    viewportHeight = a.viewportHeight
  let scrollTop = a.valueInPx // 滚动的位置
  ...
  const outerRate = (scrollTop - y) / ((_bottom > invisibleHeight ? invisibleHeight : _bottom) - y)
...
<template>
  <section ref="view" class="top"></section>
</template>
